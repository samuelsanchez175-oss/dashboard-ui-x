...
import { canUseYoutubeSearch, type ConfigStatus } from '../../lib/agent-farm-api'
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <h2 className="text-lg font-semibold text-white tracking-tight">Farm status</h2>
      ...
      {live.config && (
        <Card className="p-4 border-white/10">
          <h3 className="text-sm font-semibold text-white">Integration keys (server `.env`)</h3>
            ...
            {(
              ...
              <li
                ...
                className={`flex items-center gap-2 text-[11px] font-semibold uppercase tracking-wide px-2 py-1 rounded-md border ${
                  on ? 'border-emerald-500/30 text-emerald-200 bg-emerald-500/10' : 'border-zinc-600/50 text-zinc-500 bg-zinc-900/40'
                }`}
              >
                <img
      ...
      {live.config?.RSS_FEED_URLS && (
        <Card className="p-5 border-cyan-500/15">
          <div className="flex items-center justify-between gap-2 flex-wrap">
            <h3 className="text-sm font-semibold text-white">RSS headlines</h3>
            {live.rssLoading && <span className="text-xs text-zinc-500">Loading…</span>}
          </div>
          <p className="text-xs text-zinc-500 mt-1">{live.rssMeta ?? '—'}</p>
            ...
            {live.rssItems.slice(0, 8).map((it, i) => (
              <li key={`${it.link}-${i}`}>
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="text-cyan-200/90 hover:text-white font-medium line-clamp-2"
                >
                  {it.title}
                </a>
                <p className="text-[11px] text-zinc-500 mt-0.5 truncate" title={it.feedUrl}>
      ...
      {ytReady && (
        <Card className="p-5 border-violet-500/15">
          <h3 className="text-sm font-semibold text-white">YouTube search (live)</h3>
          <p className="text-xs text-zinc-500 mt-1">YouTube Data API v3 search — results stay client-safe (titles + ids).</p>
          <div className="mt-4 flex flex-wrap gap-2">
            <input
              ...
              placeholder="Query…"
              className="flex-1 min-w-[180px] rounded-lg border border-white/10 bg-zinc-900/80 px-3 py-2 text-sm text-white placeholder:text-zinc-600 focus:outline-none focus:ring-2 focus:ring-violet-500/40"
            />
            <button
              type="button"
          ...
          <ul className="mt-4 space-y-3 text-sm">
            {live.ytItems.map(v => (
              <li key={v.videoId ?? v.title} className="border border-white/5 rounded-lg p-3 bg-black/20">
                <p className="font-medium text-white">{v.title}</p>
                <p className="text-xs text-zinc-500 mt-1">{v.channelTitle}</p>
                <p className="text-xs text-zinc-400 mt-2 line-clamp-2">{v.description}</p>
                {v.videoId && (
                  <a
                    className="text-[11px] text-violet-300 hover:text-violet-200 mt-2 inline-block"
        ...
        <>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <Card className="p-4" glowColor="blue">
              <p className="text-xs uppercase tracking-wider text-zinc-500 font-medium">Active agents</p>
              <p className="text-2xl font-semibold text-white mt-1 tabular-nums">{farmStatus.activeAgents}</p>
            </Card>
            <Card className="p-4" glowColor="purple">
              <p className="text-xs uppercase tracking-wider text-zinc-500 font-medium">Queued jobs</p>
              <p className="text-2xl font-semibold text-white mt-1 tabular-nums">{farmStatus.queuedJobs}</p>
            </Card>
            <Card className="p-4" glowColor="green">
              <p className="text-xs uppercase tracking-wider text-zinc-500 font-medium">Est. monthly burn</p>
              <p className="text-2xl font-semibold text-white mt-1 tabular-nums">${usageStats.estimatedCostUsd.toFixed(0)}</p>
              <p className="text-[11px] text-zinc-500 mt-1">Tokens + provider fees (mock)</p>
            </Card>
            <Card className="p-4" glowColor="orange">
              <p className="text-xs uppercase tracking-wider text-zinc-500 font-medium">MRR (mock)</p>
              <p className="text-2xl font-semibold text-white mt-1 tabular-nums">${revenueMock.mrr.toLocaleString()}</p>
          ...
          <div className="grid gap-4 lg:grid-cols-2">
              ...
              <div className="mt-4 flex items-end gap-2 h-36">
                {revenueMock.series.map(row => (
                  <div key={row.label} className="flex-1 flex flex-col items-center gap-2">
                    <div
                      className="w-full rounded-md bg-gradient-to-t from-violet-600/80 to-fuchsia-500/60 min-h-[8px] transition-all"
                      style={{ height: `${(row.value / maxBar) * 100}%` }}
            ...
            <Card className="p-5" glowColor="blue">
              <h3 className="text-sm font-semibold text-white flex items-center gap-2">
                <MessageSquare size={16} className="text-violet-400/90" />
                Next actions / alerts
              </h3>
...
function BeatAnalysisPanel({ mockEnabled }: { mockEnabled: boolean }) {
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <div>
        <h2 className="text-lg font-semibold text-white tracking-tight">Beat analysis agent</h2>
        <p className="text-sm text-zinc-400 mt-1">
          Upload FLAC, WAV, or MP3 — harmonic structure for sound design
          {mockEnabled ? ' (mock result below).' : '.'}{' '}
          {!mockEnabled && (
            <span className="text-zinc-500">Sample analysis is hidden while Mock data is off.</span>
          )}
        </p>
      </div>

      <Card className="p-5">
        <label htmlFor={inputId} className="block text-xs font-medium uppercase tracking-wider text-zinc-500">
          Audio file
        </label>
        <div className="mt-3 flex flex-wrap items-center gap-3">
          <input
            ...
            onChange={onFile}
            className="text-sm text-zinc-300 file:mr-3 file:rounded-lg file:border-0 file:bg-violet-600 file:px-3 file:py-2 file:text-xs file:font-semibold file:text-white hover:file:bg-violet-500 cursor-pointer"
          />
          {fileName && (
            <span className="text-sm text-zinc-400 truncate max-w-[220px]" title={fileName}>
              Selected: {fileName}
            </span>
          )}
        </div>
      ...
      {mockEnabled ? (
        <Card className="p-5 border-violet-500/20">
          <h3 className="text-sm font-semibold text-white">Mock analysis result</h3>
          <dl className="mt-4 grid gap-3 sm:grid-cols-2 text-sm">
            <div>
              <dt className="text-zinc-500">Key / mode</dt>
              <dd className="text-white font-medium">{beatAnalysisMock.key}</dd>
            </div>
            <div>
              <dt className="text-zinc-500">Tempo</dt>
              <dd className="text-white font-medium">{beatAnalysisMock.bpm} BPM</dd>
            </div>
            <div className="sm:col-span-2">
              <dt className="text-zinc-500">Chord skeleton</dt>
              <dd className="text-violet-200 font-mono text-sm mt-1">{beatAnalysisMock.chords.join(' → ')}</dd>
            </div>
            <div className="sm:col-span-2">
              <dt className="text-zinc-500">Harmonic rhythm</dt>
              <dd className="text-white">{beatAnalysisMock.harmonicRhythm}</dd>
            </div>
          </dl>
          <ul className="mt-4 space-y-2 text-sm text-zinc-300 border-t border-white/10 pt-4">
...
function CustomerSupportPanel({ mockEnabled }: { mockEnabled: boolean }) {
  if (!mockEnabled) {
    return (
      <div className="space-y-6">
        <div>
          <h2 className="text-lg font-semibold text-white tracking-tight">Customer support agent</h2>
          <p className="text-sm text-zinc-400 mt-1">
    ...
    <div className="space-y-6">
      ...
      <Card className="overflow-hidden">
        <div className="px-4 py-3 border-b border-white/10 flex items-center justify-between">
          <span className="text-xs font-semibold uppercase tracking-wider text-zinc-500">Open queue</span>
          <span className="text-xs text-zinc-400">{supportQueueMock.length} tickets (mock)</span>
        </div>
...
function RevenuePanel({ mockEnabled }: { mockEnabled: boolean }) {
  ...
  if (!mockEnabled) {
    return (
      <div className="space-y-6">
        <div>
          <h2 className="text-lg font-semibold text-white tracking-tight">Revenue tracking agent</h2>
          <p className="text-sm text-zinc-400 mt-1">Metrics cards and trend bars — mock series for UI scaffolding.</p>
        </div>
        <AgentFarmMockOffPlaceholder />
      </div>
    )
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <Card className="p-4">
          ...
          <p className="text-xl font-semibold text-white mt-1">${revenueMock.mrr.toLocaleString()}</p>
        </Card>
        <Card className="p-4">
          <p className="text-xs text-zinc-500 uppercase tracking-wider">Growth</p>
        ...
        <Card className="p-4">
          ...
          <p className="text-xl font-semibold text-white mt-1">$42k</p>
          <p className="text-[11px] text-zinc-500 mt-1">mock weighted</p>
        </Card>
      </div>
      <Card className="p-5">
        <h3 className="text-sm font-semibold text-white">Six-month trend</h3>
        <div className="mt-4 flex items-end gap-3 h-40">
          {revenueMock.series.map(row => (
            <div key={row.label} className="flex-1 flex flex-col items-center gap-2">
              <span className="text-[11px] text-zinc-400">${(row.value / 1000).toFixed(1)}k</span>
              <div
                className="w-full rounded-lg bg-gradient-to-t from-cyan-600/70 to-violet-500/50"
                style={{ height: `${(row.value / maxBar) * 100}%`, minHeight: 8 }}
              />
              <span className="text-[10px] text-zinc-500">{row.label}</span>
            </div>
          ))}
...
function MarketResearchPanel({ mockEnabled }: { mockEnabled: boolean }) {
  if (!mockEnabled) {
    return (
      <div className="space-y-6">
        <div>
          <h2 className="text-lg font-semibold text-white tracking-tight">Market research agent</h2>
          <p className="text-sm text-zinc-400 mt-1">Keyword and trend snapshots — replace with search APIs or LLM research.</p>
        </div>
        <AgentFarmMockOffPlaceholder />
      </div>
    )
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-white/5">
            {marketResearchMock.keywords.map(k => (
              <tr key={k.term} className="hover:bg-white/[0.03]">
                <td className="px-4 py-3 text-white font-medium">{k.term}</td>
                <td className="px-4 py-3 text-zinc-400">{k.volume}</td>
                <td className="px-4 py-3 text-emerald-400">{k.trend}</td>
                <td className="px-4 py-3 text-zinc-400">{k.intent}</td>
              </tr>
            ))}
          </tbody>
        </table>
...
function SEOPanel({ mockEnabled }: { mockEnabled: boolean }) {
  if (!mockEnabled) {
    return (
      <div className="space-y-6">
        <div>
          <h2 className="text-lg font-semibold text-white tracking-tight">SEO agent</h2>
          <p className="text-sm text-zinc-400 mt-1">Recommendation backlog — wire Search Console / crawler later.</p>
        </div>
        <AgentFarmMockOffPlaceholder />
      </div>
    )
        ...
        {seoRecommendationsMock.map((r, i) => (
          <Card key={i} className="p-4 flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex gap-2">
              ...
              <span className="text-[10px] uppercase font-semibold px-2 py-0.5 rounded-full bg-zinc-500/20 text-zinc-300">
                Effort: {r.effort}
              </span>
            </div>
          </Card>
...
function SocialPanel({ mockEnabled }: { mockEnabled: boolean }) {
  if (!mockEnabled) {
    return (
      <div className="space-y-6">
        <div>
          <h2 className="text-lg font-semibold text-white tracking-tight">Social media manager</h2>
          <p className="text-sm text-zinc-400 mt-1">Queue / calendar placeholder — connect publisher APIs when ready.</p>
        </div>
        <AgentFarmMockOffPlaceholder />
      </div>
    )
      ...
      <div className="grid gap-4 md:grid-cols-3">
        {socialCalendarMock.map(day => (
          <Card key={day.day} className="p-4">
            <p className="text-xs font-semibold uppercase tracking-wider text-violet-300">{day.day}</p>
...
function AutomationsPanel() {
  return (
    <div className="space-y-6 max-w-2xl">
      <div>
        <h2 className="text-lg font-semibold text-white tracking-tight">Automations</h2>
        <p className="text-sm text-zinc-400 mt-1">
      ...
      <Card className="p-6 border-dashed border-white/20">
        <div className="flex flex-col items-center text-center gap-3 py-8">
          <Workflow className="text-zinc-500" size={40} strokeWidth={1.25} />
          ...
          <button
            type="button"
            disabled
            className="mt-2 rounded-lg bg-white/10 px-4 py-2 text-xs font-semibold text-zinc-400 cursor-not-allowed"
          >
...
function StatusPill({ configured }: { configured: boolean }) {
  ...
  return (
    <span className="text-[11px] font-semibold uppercase tracking-wide px-2 py-0.5 rounded-full bg-zinc-500/20 text-zinc-400">
      Not set
    </span>
  )
}
...
function IntegrationEnvLogo({
  ...
  return (
    <div
      className={`relative h-11 w-11 shrink-0 rounded-lg bg-white p-1.5 ring-1 ring-white/15 ${className}`}
    >
...
function IntegrationMarketplaceHex({ url, index }: { url: string; index: number }) {
  return (
    <div
      className="relative h-14 w-14 shrink-0 bg-zinc-100 p-2 shadow-sm ring-2 ring-white/10"
...
function IntegrationsPanel() {
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <h2 className="text-lg font-semibold text-white tracking-tight">API integrations</h2>
          <p className="text-sm text-zinc-400 mt-1">
      ...
      <div className="grid gap-3 sm:grid-cols-2">
          ...
          <div className="flex gap-3">
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center justify-between gap-2">
                <p className="text-sm font-medium text-white">YOUTUBE_API_KEY</p>
                <StatusPill configured={status?.YOUTUBE_API_KEY ?? false} />
              </div>
              <p className="text-xs text-zinc-500 mt-2">Preferred for YouTube Data API v3.</p>
            </div>
          </div>
        </Card>
        <Card className="p-4">
          <div className="flex gap-3">
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center justify-between gap-2">
                <p className="text-sm font-medium text-white">GOOGLE_API_KEY</p>
        ...
        <Card className="p-4">
          <div className="flex gap-3">
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center justify-between gap-2">
                <p className="text-sm font-medium text-white">GEMINI_API_KEY</p>
                <StatusPill configured={status?.GEMINI_API_KEY ?? false} />
              </div>
              <p className="text-xs text-zinc-500 mt-2">Generative Language API (Google AI).</p>
            </div>
          </div>
        </Card>
        <Card className="p-4">
          <div className="flex gap-3">
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center justify-between gap-2">
                <p className="text-sm font-medium text-white">RSS_FEED_URLS</p>
                <StatusPill configured={status?.RSS_FEED_URLS ?? false} />
              </div>
              <p className="text-xs text-zinc-500 mt-2">Comma-separated feed URLs (server-side fetch only).</p>
            </div>
          </div>
        </Card>
      </div>
      ...
      <Card className="p-5 border-white/10">
        <p className="text-sm font-medium text-white mb-1">Integration ecosystem</p>
        <p className="text-xs text-zinc-500 mb-4">Brand icons for services you can wire into the farm.</p>
      ...
      <Card className="p-4 sm:col-span-2">
        <p className="text-sm font-medium text-white mb-3">Probe BFF routes</p>
        <div className="flex flex-wrap gap-x-3 gap-y-2">
          <button
            type="button"
            disabled={probeBusy}
            onClick={() => void runProbe('Health', '/api/health')}
            className="text-xs font-semibold text-violet-300 hover:text-violet-200 disabled:opacity-40"
          >
            GET /api/health
          </button>
          <button
      ...
      {(probeLabel ?? probeBody) && (
        <Card className="p-4">
          ...
          {probeLabel && <p className="text-sm text-white mt-1">{probeLabel}</p>}
          {probeBody && (
            <pre className="mt-3 text-[11px] text-zinc-400 overflow-x-auto whitespace-pre-wrap break-all max-h-64 overflow-y-auto font-mono bg-black/30 rounded-lg p-3 border border-white/5">
              {probeBody}
            </pre>
          )}
        </Card>
...
function UsagePanel({ live, mockEnabled }: { live: AgentFarmLive; mockEnabled: boolean }) {
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-lg font-semibold text-white tracking-tight">Usage & cost</h2>
        <p className="text-sm text-zinc-400 mt-1">
          {mockEnabled ? (
            <>
      ...
      {live.config?.GEMINI_API_KEY && (
        <Card className="p-4 border-fuchsia-500/20">
          <p className="text-xs font-semibold uppercase tracking-wider text-zinc-500">Gemini (live ping)</p>
          {live.geminiLoading ? (
            <p className="text-sm text-zinc-400 mt-2">Checking Generative Language API…</p>
          ) : (
            <>
              <p className="text-sm text-white mt-2">{live.geminiStatus ?? '—'}</p>
              {live.geminiDetail && (
                <p className="text-xs text-zinc-500 mt-2 font-mono break-all">{live.geminiDetail}</p>
              )}
            </>
          )}
        </Card>
        ...
        <>
          <div className="grid gap-4 sm:grid-cols-3">
            <Card className="p-4">
              ...
              <p className="text-2xl font-semibold text-white mt-1 tabular-nums">{usageStats.totalRuns.toLocaleString()}</p>
            </Card>
            <Card className="p-4">
            ...
            <Card className="p-4">
              <p className="text-xs text-zinc-500 uppercase tracking-wider">Est. cost</p>
              <p className="text-2xl font-semibold text-white mt-1 tabular-nums">${usageStats.estimatedCostUsd}</p>
            </Card>
          </div>
          <Card className="overflow-hidden">
            <div className="px-4 py-3 border-b border-white/10">
              <span className="text-xs font-semibold uppercase tracking-wider text-zinc-500">By agent</span>
            </div>
            <ul className="divide-y divide-white/5">
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h2 className="text-lg font-semibold text-white tracking-tight flex items-center gap-2">
            ...
            <span className="text-zinc-100">Environment area</span>
          </h2>
          <p className="text-sm text-zinc-400 mt-1">
            Local workspace — prompts, checklists, and notes for this slice of the farm. Open it from the farm tile grid
            with a <span className="font-mono text-violet-300">+</span> prefix on custom tiles.
          </p>
        </div>
        <button
          type="button"
      ...
      <Card className="p-5" glowColor="purple">
        <label className="block space-y-2">
          ...
          <input
            ...
            className="w-full rounded-lg border border-white/10 bg-zinc-900/80 px-3 py-2 text-sm text-white placeholder:text-zinc-600 focus:outline-none focus:ring-2 focus:ring-violet-500/40"
          />
        </label>
        <label className="block space-y-2 mt-5">
          ...
          <textarea
            ...
            rows={14}
            className="w-full resize-y min-h-[200px] rounded-lg border border-white/10 bg-zinc-900/80 px-3 py-2 text-sm text-zinc-200 placeholder:text-zinc-600 focus:outline-none focus:ring-2 focus:ring-violet-500/40 font-mono leading-relaxed"
          />
        </label>
      </Card>
    </div>
...
export default function AgentFarm() {
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0 bg-zinc-950 text-zinc-200">
      <header className="shrink-0 border-b border-white/10 px-6 py-5 bg-gradient-to-r from-zinc-950 via-zinc-900 to-violet-950/40">
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            <p className="text-[11px] uppercase tracking-[0.2em] text-violet-400/90 font-semibold">Agent swarm</p>
            <h1 className="text-2xl font-semibold text-white tracking-tight mt-1">Agent Farm</h1>
            <p className="text-sm text-zinc-400 mt-1 max-w-xl">
              Extend the environment with <span className="font-mono text-violet-300">+</span> workspaces — each keeps its
              own title and notes in this browser. Pick a tile below to open a section; built-in tools pull live RSS, YouTube,
              and Gemini when your server <code className="text-[11px] text-zinc-500">.env</code> is set.
            </p>
          </div>
          <div className="flex flex-wrap items-center justify-end gap-3 text-xs text-zinc-500">
            ...
            <div className="flex items-center gap-2">
              ...
              Sidebar: <span className="text-zinc-300 font-medium">Agent Farm</span>
            </div>
          </div>
        </div>
      ...
      >
        {!farmExpanded ? (
          <nav className="max-w-6xl mx-auto" aria-label="Agent farm sections">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
              {TABS.map(t => {
                ...
                return (
                  <button
                    ...
                    className={`
                      group flex flex-col items-start gap-3 rounded-xl border p-4 text-left transition-all min-h-[112px]
                      ...
                      ${isActiveMemory
                        ? 'border-violet-500/70 bg-violet-950/35 shadow-lg shadow-violet-950/20'
                        : 'border-white/10 bg-white/[0.04] hover:bg-white/[0.07] hover:border-violet-400/25'
                      }
                    `}
                  >
                    <Icon
                      size={22}
                      strokeWidth={1.75}
                      className={`shrink-0 ${isActiveMemory ? 'text-violet-300' : 'text-violet-400/90 group-hover:text-violet-300'}`}
                      aria-hidden
                    />
                    <div className="min-w-0 space-y-1">
                      <p className="text-[14px] font-semibold text-white leading-tight">{t.label}</p>
                      <p className="text-[11px] text-zinc-500 leading-snug">{t.shortLabel}</p>
                    </div>
                  </button>
                )
              })}
                ...
                return (
                  <button
                    ...
                    className={`
                      ...
                      ${isActiveMemory
                        ? 'border-emerald-500/70 bg-emerald-950/30 shadow-lg shadow-emerald-950/25'
                        : 'border-emerald-500/20 bg-emerald-950/15 hover:bg-emerald-950/25 hover:border-emerald-400/35'
                      }
                    `}
                  >
                    <div className="flex items-center gap-2">
                    ...
                    <div className="min-w-0 space-y-1 w-full">
                      <p className="text-[14px] font-semibold text-white leading-tight truncate">{area.label || '+ Untitled'}</p>
              ...
              <button
                ...
                onClick={addEnvArea}
                className="
                  flex flex-col items-center justify-center gap-2 rounded-xl border border-dashed border-violet-400/35 min-h-[112px] p-4
                  text-[13px] font-medium text-violet-300/90 hover:bg-violet-500/10 hover:border-violet-400/55 transition-colors
                  focus:outline-none focus-visible:ring-2 focus-visible:ring-violet-500 focus-visible:ring-offset-2 focus-visible:ring-offset-zinc-950
                "
                aria-label="Add environment workspace"
              >
                <Plus size={22} strokeWidth={2} className="shrink-0 text-violet-400" />
          ...
          <div className="max-w-6xl mx-auto space-y-5">
            <div
              className="sticky top-0 z-10 -mx-6 lg:-mx-8 px-6 lg:px-8 py-3 mb-1 flex flex-wrap items-center gap-3 border-b border-white/10 bg-zinc-950/90 backdrop-blur-md supports-[backdrop-filter]:bg-zinc-950/75"
            >
              <button
                type="button"
                onClick={() => setFarmExpanded(false)}
                className="inline-flex items-center gap-1.5 rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-xs font-semibold text-zinc-200 hover:bg-white/10 hover:text-white transition-colors"
              >
