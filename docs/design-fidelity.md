...
## Family F — CDL quiz immersion

**Zones:** CDL hub + quiz pages (intentional visual exception from the main dashboard).

**Before merge, verify:**

- Immersion treatments (full-bleed, stronger contrast) are **intentional** and scoped to CDL routes only — do not leak tokens into Family A/B shells.
- Quiz controls remain keyboard-accessible; focus rings visible in both themes.
- Progress / score UI still meets minimum contrast for small text.
